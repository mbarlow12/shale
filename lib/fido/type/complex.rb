...
module Fido
  module Type
    ...
    # It serves as a base type class for @see Fido::Mapper
    #
    # @api private
    class Complex < Value
      class << self
        %i[hash json yaml toml csv].each do |format|
          class_eval(<<-RUBY, __FILE__, __LINE__ + 1)
            ...
            def of_#{format}(hash, only: nil, except: nil, context: nil)
              ...
              grouping = Fido::Mapping::Group::DictGrouping.new
              delegates = Fido::Mapping::Delegates.new
        ...
        def from_csv(csv, only: nil, except: nil, context: nil, headers: false, **csv_options)
          data = Fido.csv_adapter.load(csv, **csv_options.merge(headers: csv_mapping.keys.keys))
        ...
        def to_csv(instance, only: nil, except: nil, context: nil, headers: false, **csv_options)
          ...
          Fido.csv_adapter.dump(data, **csv_options.merge(headers: cols))
        end

        # Convert XML document to Object
        #
        # @param [Fido::Adapter::<XML adapter>::Node] element
        # @param [Array<Symbol>] only
        # @param [Array<Symbol>] except
        # @param [any] context
        #
        ...
        def of_xml(element, only: nil, except: nil, context: nil)
          ...
          grouping = Fido::Mapping::Group::XmlGrouping.new
          delegates = Fido::Mapping::Delegates.new
        ...
        # @param [Fido::Adapter::<xml adapter>::Document, nil] doc Object to convert
        # @param [Array<Symbol>] only
        # @param [Array<Symbol>] except
        # @param [any] context
        #
        ...
        def as_xml(
          ...
          unless doc
            doc = Fido.xml_adapter.create_document(version)
        ...
        def validate_toml_adapter
          raise AdapterError, TOML_ADAPTER_NOT_SET_MESSAGE unless Fido.toml_adapter
        end
        ...
        def validate_xml_adapter
          raise AdapterError, XML_ADAPTER_NOT_SET_MESSAGE unless Fido.xml_adapter
        end
        ...
        #
        # @param [Fido::Mapping::Descriptor::Dict] mapping
        #
        # @raise [AttributeNotDefinedError]
        # @raise [NotAFidoMapperError]
        #
        # @return [Hash<Symbol, Fido::Attribute>]
        #
        # @api private
        def get_receiver_attributes(mapping)
          ...
          unless receiver_attribute.type < Mapper
            msg = "attribute '#{mapping.receiver}' is not a descendant of Fido::Mapper type"
            raise NotAFidoMapperError, msg
          end
        ...
        #
        # @param [any] instance
        # @param [Fido::Attribute] receiver_attribute
        #
        # @return [Array]
        #
        # @api private
